"""GBM v4: extended features + pulse-phase indicators.

Adds 3 one-hot phase indicators: in_rampup, in_flattop, in_rampdown.
Lets the classifier learn phase-conditional thresholds — e.g., 'ignore
Kuramoto rise during controlled ramp-down'.

27 features total (24 base + 3 phase).
"""
...
def sample_dense(shot, features, dt, baseline_window_ms=50.0,
    ...
    """Dense sampler — also samples NON-DISRUPTIVE ramp-down windows as
    negatives so the in_rampdown phase indicator cannot leak the label.
    """
    ...
    if shot.is_disruptive and shot.t_disrupt_s is not None:
        ...
        # Non-disruptive: include the FULL shot length up to the data end,
        # so the model sees in_rampdown=1 windows labelled as 0 (negative).
        ft_end = len(features)
...
def main():
    print("MAST GBM v4 (extended + pulse-phase indicators, 27 features)",
          flush=True)
    print("=" * 70, flush=True)
    ...
    fold_results = {f"gbm4_p{thr:.2f}": [] for thr in score_thresholds}
    fold_importances = []
    feat_names = None
    ...
    for fi, test_idx in enumerate(folds):
        ...
        for s in shots:
            ...
            f, fn = compute_phase_aware_features(
            ...
            feat_names = fn
        print(f"    phase-aware features: {time.time() - t1:.1f}s", flush=True)
        ...
        # Avoid /0 on the phase one-hot columns (sigma can be near 0)
        ...
        for s in test_shots:
            ...
            for thr in score_thresholds:
                ...
                r = mask_to_analysis_window(r, s.time_s,
                                              ...
                                              s.t_disrupt_s)
                fold_results[f"gbm4_p{thr:.2f}"].append(
    ...
    summary = []
    for thr in score_thresholds:
        ds = score_dataset(fold_results[f"gbm4_p{thr:.2f}"])
        ttd = (f"{ds.median_ttd_ms:.1f}" if ds.median_ttd_ms is not None
               else "n/a")
        print(f"  GBM v4 p={thr:.2f}: TPR={ds.tpr*100:>4.0f}% "
    ...
    print()
    print("Top-15 importances:", flush=True)
    ...
    out = Path(__file__).parent / "mast_gbm_v4_results.json"
    out.write_text(json.dumps({
        "dataset": "MAST disruption (Sharma 2025, Zenodo 16032053)",
        "n_shots": len(shots), "n_folds": 5,
        "model": "GBM v4: 250 trees, depth=5, 27 features (24 base + 3 phase)",
