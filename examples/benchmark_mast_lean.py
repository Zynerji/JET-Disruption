"""Lean v5: 9 features designed from failure analysis findings.

Drops absolute-R features and per-channel peakz (both noise per failure
analysis). Adds weighted_consensus dynamics (rate, duration above
threshold, integral) on the dominant 200ms scale.
"""
...
from jet_disruption.lean_features import (
    compute_lean_features, LEAN_FEATURE_NAMES,
)
...
def main():
    print("MAST GBM LEAN v5 (9 features from failure analysis)", flush=True)
    print("=" * 70, flush=True)
    ...
    fold_results = {f"lean_p{thr:.2f}": [] for thr in score_thresholds}
    fold_importances = []
    ...
    for fi, test_idx in enumerate(folds):
        ...
        for s in shots:
            ...
            feats_cache[s.shot_id] = compute_lean_features(
                s.diagnostics, dt, wr.weights, s.time_s,
                ramp_up_tmax=getattr(s, "ramp_up_tmax", None),
                flat_top_tmax=getattr(s, "flat_top_tmax", None),
            )
        print(f"    lean features: {time.time() - t1:.1f}s", flush=True)
        ...
        for s in test_shots:
            ...
            for thr in score_thresholds:
                ...
                r = mask_to_analysis_window(r, s.time_s,
                                              ...
                                              s.t_disrupt_s)
                fold_results[f"lean_p{thr:.2f}"].append(
    ...
    summary = []
    for thr in score_thresholds:
        ds = score_dataset(fold_results[f"lean_p{thr:.2f}"])
        ttd = (f"{ds.median_ttd_ms:.1f}" if ds.median_ttd_ms is not None
               else "n/a")
        print(f"  lean p={thr:.2f}: TPR={ds.tpr*100:>4.0f}% "
    ...
    for i in order:
        print(f"  {LEAN_FEATURE_NAMES[i]:>40s}: {imp_mean[i]:.3f}", flush=True)

    out = Path(__file__).parent / "mast_lean_results.json"
    out.write_text(json.dumps({
        "dataset": "MAST disruption (Sharma 2025, Zenodo 16032053)",
        "n_shots": len(shots), "n_folds": 5,
        "model": "GBM v5 lean: 9 features designed from failure analysis",
        "feature_names": list(LEAN_FEATURE_NAMES),
