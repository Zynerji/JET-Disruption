"""GBM v2: denser sampling + tighter precursor window + stronger config.

Changes from v1:
  - precursor window 100ms -> 50ms (sharper precursor)
  - sampling stride 5ms -> 2ms (5x more positive samples per shot)
  - GBM: 250 trees, depth=5, lr=0.05, subsample=0.7
  - regularization via min_samples_leaf=10
"""
...
def sample_dense(shot, features, dt,
                   baseline_window_ms=50.0,
                   precursor_lead_ms=50.0,
                   precursor_min_ahead_ms=2.0,
                   quiescent_buffer_ms=150.0,
                   stride_ms=2.0):
    """Dense sampling: 2ms stride; shorter precursor window."""
...
def main():
    print("MAST GBM v2 (dense sampling + stronger model)", flush=True)
    print("=" * 70, flush=True)
    ...
    fold_results = {f"gbm2_p{thr:.2f}": [] for thr in score_thresholds}
    fold_importances = []
    ...
    for fi, test_idx in enumerate(folds):
        ...
        print(f"    features: {time.time() - t1:.1f}s", flush=True)

        # Dense training windows
        Xs, ys = [], []
        ...
        print(f"    training samples: {len(y_all)} (pos={n_pos}, neg={n_neg})",
              flush=True)
        ...
        for s in test_shots:
            ...
            for thr in score_thresholds:
                ...
                r = mask_to_analysis_window(r, s.time_s,
                                              ...
                                              s.t_disrupt_s)
                fold_results[f"gbm2_p{thr:.2f}"].append(
    ...
    summary = []
    for thr in score_thresholds:
        ds = score_dataset(fold_results[f"gbm2_p{thr:.2f}"])
        ttd = (f"{ds.median_ttd_ms:.1f}" if ds.median_ttd_ms is not None
               else "n/a")
        print(f"  GBM v2 p={thr:.2f}: TPR={ds.tpr*100:>4.0f}% "
    ...
    print()
    print("Mean importances:", flush=True)
    imp_mean = np.mean(np.asarray(fold_importances), axis=0)
    order = np.argsort(-imp_mean)
    for i in order:
        print(f"  {MULTISCALE_FEATURE_NAMES[i]:>26s}: {imp_mean[i]:.3f}",
              flush=True)

    out = Path(__file__).parent / "mast_gbm_v2_results.json"
    out.write_text(json.dumps({
        "dataset": "MAST disruption (Sharma 2025, Zenodo 16032053)",
        "n_shots": len(shots), "n_folds": 5,
        "model": "GBM v2: 250 trees, depth=5, lr=0.05, subsample=0.7, dense sampling",
